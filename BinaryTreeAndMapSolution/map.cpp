...
int main(int argc, char **argv) {
    ...
    while (getline(data_set, one_line, '\n')) {
        ...
        employee_map[new_employee->get_id()] = new_employee;    
    ...
    while (getline(operations_file_var, one_line, '\n')) {
        ...
        if (operation_name == "ADD") {
            ...
            if (employee_map.find(id_to_update_int) == employee_map.end()) {
                cout << "ERROR: An invalid ID to update" << "\n";
                continue;
            }
            
            Employee* emp_to_update = employee_map[id_to_update_int]; 

            emp_to_update->set_salary(stoi(updated_salary));
            emp_to_update->set_department(stoi(updated_department));
            ...
            if (employee_map.find(id_to_delete_int) == employee_map.end()) {
                cout << "ERROR: An invalid ID to delete" << "\n";
                continue;
            }

            delete employee_map[id_to_delete_int];
            employee_map.erase(id_to_delete_int);
    ...
    ofstream output_file_var("map_solution.csv");
    output_file_var << first_line << "\n";

    for (auto const& pair : employee_map) {
        output_file_var << pair.second->get_id() << ";" << pair.second->get_salary() << ";" << pair.second->get_department() << "\n";
    }
